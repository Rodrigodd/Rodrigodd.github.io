...
reach a got point, where it pass a lot of tests (comparable to some of the most
...
time that I searching how a emulator works, and one of the firts descriptions I
...
implemented (with exception of the interpreter).

## Interpreter x Just-In-Time compilation

- Make clear that only the CPU need to be interpreted/compiled.
- Explain how a interpreter works in the Game Boy.
- Explain in a simple way how JIT compilation works.
- Explain all the cool techiniques you can use to optimize the code.
- Explain that the CPU also need to handle the interrupts.
- Explain there is not much you can do in the intepreter for handling in the
  CPU.
- Explain how the straigh forward method of handling interrupts breaks all the
  cool optimizations.
- Explain how can this be mitigated by predicting when a interrupt will happen.
- Explain that before that we need to implement Lazy Update.

A emulator has the job of reproducing the behaviour of a computer system in
...
There are two main ways of emulating the CPU.

### A very basic interpreter

The easest way of emulating the CPU of some system is by implementing a
...
byte (Game Boy is a 8-bit system), from the memory at the address pointed by the
program counter (PC), where after each read the PC is incremented. To decode the
instruction only the first byte is need for most instructions, and because there is
only 255 values for a byte, we can just use a look up table. The Game Boy CPU
...
execute the code is very behind the theorical speed that yours computer CPU
execute the code.
...
that the host CPU does in parallel. Not only that, the intepreter need to decode
the same instruction multiple times.

But that is not a problem, at least not for the Game Boy, whose CPU runs with a
clock speed of just a little more than 1 MHz, while you are may be reading this
...
2.6 GHz, using only the interpreter.

Well, but this is still 82 times slower than the theorical speed that my CPU can
execute the code (if you assume the same ammount of instructions per
instruction, not that reasonable). And there is a very interesting technique
that can be used to improve the performance of a emulator: **recompilation**.
...
systems, you can not recompiled all the code containing in a program before hand
...
This `compile_instruction` function reads and decodes a instruction, and then
...
instruction in Rust][inc_b_playground], and then dissasembled it with `objdump
-d`.)_
...
emmiting the machine code (this crates have a macro that translate assembly code
to machine code at compile time), so the code is a lot more maintanable than the
...
emulator is the omission of unecessary flag calculations (directly inspired by
the one described in [Brook Heisler's post][nes_jit].

If you tried to read what the assembly code is doing, you may have noticed that only
instructions are used for increment the value of register B, and the remaning 10
are being used for updating the conditional flags in the register F. But the
value of F may be overwritten by the next instructions before it could be read
from a conditional jump instruction for example. In that scenario, updating the
value of F does not affect the behaviour of the emulation, so it is a waste to
compute.

If we compiled all these instructions together, we can detect that kind of
scenario, and ommited the flag calculation, greatly reducing the ammount of code
emmited!
...
reach the point where the optimizations becomes by far the most complex part of
a compiler. Instead of implementing them your self, you could use a third party
compiler insfrastructure, like [LLVM][llvm] or [Cranelift][clift], which
...
currently code, and start executing a function that handles the interrupt. For
...
Our interpreter becames something like this:
...
constantly checking for interrupts may introduce a considerable ammount of
...
possible to implement it, without sacrifising precision nor performance.
...
The JIT compiler in my emulator works this way: given the currently address in the
...
But before executing the block, it checks if a interrupt will happen during its
execution. If not, the block is executed, otherwise, we fallback to the
interpreter.

This allows us the apply all the optimizations that we want, without worrying
with interrupts (most of the part).

The disadvantage is that we still need to have a interpreter around, and the
fallback may decrease the emulator performance. But I already had a interpreter
working, so no problem in that, and only a small fraction of the instructions
need to be interpreted, so the loss of performance is not so bad. But a
...
If you want to take a look at how the implementation looks like, here are a
...
accessing a memory address that is mapped to a component, or right before a
...
clock count. The cool part is that this allows us to futher optimize the
emulation of each components by decreasing the number of times we update each
...
component responsible for transforming the tilemap and sprites data into pixels
...
the PPU every instructions, and when updating the PPU only when necessary.

![PPU emulation time](/assets/gameroy_jit/interrupt-prediction.svg)

The `draw_scan_line` function render the final result of a scanline, without
emulating the entire PPU inner workings. As is possible to see, the emulation of
...
the emulator, and that the JIT compiler would not provide a significant speed
up. But after this optimization, the CPU emulation is responsible for 63% of the
time, giving some room for the JIT compiler to shine.
...
First, it need to decided which group of instructions it will compiled. For that
...
instructions one after the other, until it reaches a unconditional branch (like
...
A important point is that I only trace blocks when the PC is pointing to a
address in the memory ROM (range 0x0000 to 0xFFFF). If it is pointing outside of
it, like the RAM, I just fallback to the interpreter. This allows me to avoid
handling self modifying code, which is a general problem for dinamic
recompilers. Also, games in general don't run much code in RAM, so is not that a
big of a loss.

Another point is that the Game Boy uses bank switching, which means that address
in the ROM range may point to different banks of memory, at diferent times. So,
a block need to be identified not only by its starting address, but also by
which bank.

After tracing a block, the compiler will compile it down to a function. So it
emmits machine code for the function prologue (align the stack, push registers,
etc.), then emmits the machine code for each instruction.
...
Instrucions that may branch, checks if the target address is inside the block,
and if is, it emmits a jump to the target address inside the block. This uses
[`dynasm`][dynasm] helper for emmiting jumps. If the target isn't in the block,
it just emmits the prologue for block (pop registers and call `ret`).

Instructions that read or write to memory emmit function calls to Rust functions
...
with immediate address may be emmited inline, if possible.

Another important point, is that writing to memory may trigger a interrupt, or
at least change its timing. This means that after each writing, the compiler
needs to emmit another check for interrupts, and exit if need. The same is true
for bank switching: a write to ROM could switch the bank that the block is
currently executing, so it need check if it did change and exit the block.

Also, because writes can trigger interrupts, and the interrupts can read the
content of flags, each write also compromisses inter instructions optimizations,
like the omission of flag calculations that I mention before.

And because there are now multiple checks for interrupts through out the block,
each check only need to make sure that if the next interrupt will not happen
before the next interrupt check, including the initial check that happens before
entering the block.

After finishing emmiting the block, the compiler moves the buffer with the
machine code to a executable memory region, and store it in a `HashMap`, that
maps `(bank, address)` to the block. Now, whenever the emulator needs to execute
this block again, it just need to get it in the `HashMap` (or compile it if it
isn't there yet).
...
First, let's the compare the speed of the interpreter and the JIT compiler. For
...
The emulator was capable of emulating Tobu Tobu Girld more than 100 times faster
...
emulation of the PPU. We also can see that Zelda has a lot less oportunities for
...
(handwavy) that the JIT compiler is about 4 times faster than the interpreter.
...
or three times, the results are not that precised. But hopefully they are good
...
I may have achieved the state-of-the-art in regards to dynamic recompilation
...
The flags is the most sub-optimal, because I am computing the value of the flag,
enconding it in the F register bitflag, and them later decoding them in the
...
Another inneficiency in my JIT compiler, is in regards to redundant
recompilations. I am currently building a entire block for each `(bank, address)`
...
introducing unecessary compilation time and memory usage.
...
check out the [its GitHub repository][gameroy].
